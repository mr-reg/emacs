		    ...
		    struct timespec mtime)
{
  ...
  FOR_EACH_TAIL_SAFE (eln_path_tail)
	...
	alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", eln_rel_name,
			   alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", Vcomp_native_version_dir, dir));
  ...
  /* Look also in preloaded subfolder of the last entry in
     `comp-eln-load-path'.  */
  dir = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("preloaded"),
			   alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", Vcomp_native_version_dir,
					      dir));
  maybe_swap_for_eln1 (src_name, alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", eln_rel_name, dir),
		       filename, fd, mtime);
#endif
}
  ...
  FOR_EACH_TAIL_SAFE (path)
   {
    ...
    if (EQ (path, just_use_str))
      ...
      filename = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", str, XCAR (path));
    if (!complete_filename_p (filename))
      /* If there are non-absolute elts in PATH (eg ".").  */
      /* Of course, this could conceivably lose if luser sets
	 default-directory to be something non-absolute...  */
      {
	filename = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", filename, BVAR (current_buffer, directory));
	if (!complete_filename_p (filename))
	      ...
	      Lisp_Object start, Lisp_Object end)
{
  ...
  if (!will_dump_p ()
    ...
    sourcename = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", sourcename, Qnil);
  ...
  if (!NILP (Vinstallation_directory))
    {
      ...
      tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("lisp"),
      ...
      if (!no_site_lisp)
        {
          tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("site-lisp"),
                                   Vinstallation_directory);
          tem1 = Ffile_accessible_directory_p (tem);
          if (!NILP (tem1))
            {
      ...
      if (NILP (Fequal (Vinstallation_directory, Vsource_directory)))
        {
          ...
          tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("src/Makefile"),
                                   Vinstallation_directory);
          ...
          tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("src/Makefile.in"),
                                   Vinstallation_directory);
...
You cannot count on them to still be there!  */);
  Vsource_directory
    = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("../"),
