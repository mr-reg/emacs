  ...
  if (nargs >= 2 && ! NILP (args[1]))
    {
	 ...
	 directory, or its unhandled equivalent ("~").  */
      infile = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", args[1], get_current_directory (false));
      CHECK_STRING (infile);
    }
  else
  ...
  if (nargs < 3)
      ...
      if (CONSP (buffer) && !EQ (XCAR (buffer), QCfile))
	{
	  if (CONSP (XCDR (buffer)))
		...
		error_file = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", stderr_file, Qnil);
	    }
      ...
      if (CONSP (buffer) && EQ (XCAR (buffer), QCfile))
	{
	  ...
	  if (CONSP (ofile))
	    ofile = XCAR (ofile);
	  CHECK_STRING (ofile);
	  output_file = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", ofile,
		  ...
		  Lisp_Object *filename_string_ptr)
{
  ...
  {
    Lisp_Object pattern = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", Vtemp_file_name_pattern, tmpdir);
    char *tempfile;
  ...
  if (!NILP (Vinstallation_directory))
    {
      ...
      Lisp_Object tem;
      tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("lib-src"),
      ...
      if (NILP (Fmember (tem, Vexec_path)))
	{
      ...
      if (data_dir == 0)
	{
	  tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("etc"),
				   Vinstallation_directory);
  ...
  if (data_dir == 0)
    {
      ...
      srcdir = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("../src/"), lispdir);

      tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("NEWS"), Vdata_directory);
      if (!NILP (Fequal (srcdir, Vinvocation_directory))
	  || NILP (Ffile_exists_p (tem)) || !NILP (Vinstallation_directory))
	{
	  Lisp_Object newdir;
	  newdir = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("../etc/"), lispdir);
	  tem = alien_rpc2("cl-emacs/elisp:elisp/expand-file-name", build_string ("NEWS"), newdir);
